    ...
    <div class="condition-container">
      <b-form class="needs-validation" novalidate v-if="show" @submit="conditionSubmitHandler"
        ...
        <b-row>
          ...
          <div class="col-md-2 pt-2 text-center">申请辅导备案时间：</div>
        ...
        <b-row>
          <b-form-group horizontal :label-cols="3" class="col-md-4" label="月报状态：" label-for="status">
...
<script>
  ...
  const _GoldenMonthList = {
    name: 'GoldenMonthList',
    created: function () {
      this._searchData();
    },
    updated: function () {
    ...
    data: function () {
      ...
      return {
        ...
        account_fields: [
          ...
          {key: 'list_place', label: '上市地'},
          {key: 'filing_time', label: '申请辅导备案时间'},
          {key: 'solution', label: '存在的困难、问题和解决进展'},
          {key: 'company_contact', label: '公司联系人、联系方式'},
          {key: 'sponsors_contact', label: '保荐机构及联系人、联系方式'},
          {key: 'audit_contact', label: '审计事务所及联系人、联系方式'},
          {key: 'law_contact', label: '律师事务所及联系人、联系方式'},
    ...
    methods: {
      ...
      actionEditHandler(item, index, event) {
        this.$router.push({name: 'golden_edit', query: {id: '1'}});
      },
  ...
  export default _GoldenMonthList;
</script>
