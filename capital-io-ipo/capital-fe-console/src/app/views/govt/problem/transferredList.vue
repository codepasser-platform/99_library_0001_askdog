...
<script>
  ...
  import {_transferredListSearch} from "../../../service/api/problem/problem-api";

  const _ProblemTransList = {
    name: 'ProblemTransList',
    created: function () {
      this._searchData();
    },
    updated: function () {
    ...
    data: function () {
      ...
      return {
        ...
        grid_fields: [
          {key: 'number', label: '序号', sortable: false},
          {key: 'company_name', label: '企业名称', sortable: false},
          {key: 'register_area', label: '注册地', sortable: false},
          {key: 'report_person', label: '申报人', sortable: false},
          {key: 'report_time', label: '申报时间', sortable: true},
          {key: 'current_node', label: '当前节点', sortable: true},
          {key: 'status', label: '状态', sortable: true,
            //<!--根据status状态code,显示不同的状态提示： 方法2 -->
            formatter: (value) => {
              let result = '';
              if (value == 'CITY_WAITING') {
                result = "待受理";
              } else {
                result = "处理中";
              }
          ...
          {key: 'trans_time', label: '流转时间', sortable: true},
    ...
    methods: {
      ...
      _searchData() {
        ...
        const _self = this;
        _transferredListSearch (this.condition_form, this.currentPage, this.perPage).then(
          function (response) {
  ...
  export default _ProblemTransList;
</script>
