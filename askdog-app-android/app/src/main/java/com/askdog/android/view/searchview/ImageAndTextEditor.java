 ...
 */
public class ImageAndTextEditor extends EditText {
    ...
    public ImageAndTextEditor(Context context) {
        super(context);
        init(context);
    }

    public ImageAndTextEditor(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ImageAndTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        Bitmap bitmap = getSmallBitmap(path, 480, 800);
        Bitmap bitmap = getSmallBitmap(path, 300, 200);
        insertBitmap(path, bitmap);
    }
    /**
     * 用集合的形式获取控件里的内容
