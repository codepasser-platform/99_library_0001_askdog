...
public class TencentLocationAgent implements LocationAgent {
    ...
    private ResponseBody analysisAddress(String ip) {
        return new LocationResponseBody(this.getRestTemplate().getForObject(locationConfiguration.getIpLocation(), String.class, ip));
    }

    private ResponseBody analysisAddress(Double lat, Double lng) {
        return new LocationResponseBody(this.getRestTemplate().getForObject(locationConfiguration.getGeoLocation(), String.class, lat, lng, EXCLUDE_POI));
    }

    private ResponseBody analysisSuggestion(Double lat, Double lng) {
