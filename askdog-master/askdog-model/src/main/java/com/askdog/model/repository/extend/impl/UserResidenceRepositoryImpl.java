...
public class UserResidenceRepositoryImpl implements ExtendedLocationRecordRepository {
    ...
    @PostConstruct
    private void init() {
        mongoTemplate.indexOps(QuestionLocation.class).ensureIndex(new GeospatialIndex("geometry").typed(GeoSpatialIndexType.GEO_2DSPHERE));
    ...
    @Override
    @Nonnull
    public List<UserLocation> findNearbyUsers(@Nonnull String userId, @Nonnull Double lat, @Nonnull Double lng, double nearbyDistance, long nearbyLimit) {
    ...
    @Override
    @Nonnull
    public List<UserResidence> findUserLocation(@Nonnull String userId, long recentLimit) {
        return mongoTemplate.aggregate(
                newAggregation(
    ...
    @Override
    @Nonnull
    public UserResidence countNearbyPoints(@Nonnull String userId, @Nonnull Double lat, @Nonnull Double lng, double rateDistance) {
        return mongoTemplate.aggregate(
                newAggregation(
