...
#define selectedTitleAndIndicatorViewColor  [UIColor whiteColor]
...
- (void)setScrollTitleArr:(NSArray *)scrollTitleArr {
    ...
    for (NSUInteger i = 0; i < self.scrollTitleArr.count; i++) {
        ...
        _staticTitleLabel.textColor = titleColor;
        _staticTitleLabel.backgroundColor = [UIColor clearColor];
    ...
    _indicatorView.SG_y = self.frame.size.height - indicatorHeight - 5;
...
- (void)scrollTitleLabelSelecteded:(UILabel *)label {
    ...
    // 颜色恢复
    _selectedLabel.textColor = titleColor;
