...
@property (nonatomic, strong) UIButton *actionButtonQuit;
...
- (void)viewDidLoad
{
    ...
    [_actionButton setTitle:@"Q" forState:UIControlStateNormal];
	...
	[_actionButton addTarget:self action:@selector(quitAction) forControlEvents:UIControlEventTouchUpInside];
	_actionButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin;
	[self.view addSubview:_actionButton];
    
    self.actionButtonQuit = [UIButton buttonWithType:UIButtonTypeCustom];
    [_actionButtonQuit setTitle:@"C" forState:UIControlStateNormal];
    [_actionButtonQuit setTitleColor:_textColor forState:UIControlStateNormal];
    [_actionButtonQuit setTitleColor:[_textColor colorWithAlphaComponent:0.5f] forState:UIControlStateHighlighted];
    _actionButtonQuit.titleLabel.font = [_actionButtonQuit.titleLabel.font fontWithSize:ACTION_BUTTON_WIDTH];
    _actionButtonQuit.frame = CGRectMake(self.view.frame.size.width - ACTION_BUTTON_WIDTH - 5 - ACTION_BUTTON_WIDTH,
                                     self.view.frame.size.height - EDITFIELD_HEIGHT - 5,
                                     ACTION_BUTTON_WIDTH, EDITFIELD_HEIGHT);
    [_actionButtonQuit addTarget:self action:@selector(infoAction) forControlEvents:UIControlEventTouchUpInside];
    _actionButtonQuit.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin;
    [self.view addSubview:_actionButtonQuit];
	
	if (_delegate)
	{
		_inputField = [[UITextField alloc] initWithFrame:CGRectMake(5, self.view.frame.size.height - EDITFIELD_HEIGHT - 5,
                                                                    self.view.frame.size.width - 15 - ACTION_BUTTON_WIDTH - ACTION_BUTTON_WIDTH,
                                                                    EDITFIELD_HEIGHT)];
